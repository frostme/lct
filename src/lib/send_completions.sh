## [@bashly-upgrade completions send_completions]
send_completions() {
  echo $'# lct completion                                           -*- shell-script -*-'
  echo $''
  echo $'# This bash completions script was generated by'
  echo $'# completely (https://github.com/bashly-framework/completely)'
  echo $'# Modifying it manually is not recommended'
  echo $''
  echo $'_lct_completions_filter() {'
  echo $'  local words="$1"'
  echo $'  local cur=${COMP_WORDS[COMP_CWORD]}'
  echo $'  local result=()'
  echo $''
  echo $'  # words the user already typed (excluding the command itself)'
  echo $'  local used=()'
  echo $'  if ((COMP_CWORD > 1)); then'
  echo $'    used=("${COMP_WORDS[@]:1:$((COMP_CWORD - 1))}")'
  echo $'  fi'
  echo $''
  echo $'  if [[ "${cur:0:1}" == "-" ]]; then'
  echo $'    # Completing an option: offer everything (including options)'
  echo $'    echo "$words"'
  echo $''
  echo $'  else'
  echo $'    # Completing a non-option: offer only non-options,'
  echo $'    # and don\'t re-offer ones already used earlier in the line.'
  echo $'    for word in $words; do'
  echo $'      [[ "${word:0:1}" == "-" ]] && continue'
  echo $''
  echo $'      local seen=0'
  echo $'      for u in "${used[@]}"; do'
  echo $'        if [[ "$u" == "$word" ]]; then'
  echo $'          seen=1'
  echo $'          break'
  echo $'        fi'
  echo $'      done'
  echo $'      ((!seen)) && result+=("$word")'
  echo $'    done'
  echo $''
  echo $'    echo "${result[*]}"'
  echo $'  fi'
  echo $'}'
  echo $''
  echo $'_lct_completions() {'
  echo $'  local cur=${COMP_WORDS[COMP_CWORD]}'
  echo $'  local compwords=()'
  echo $'  if ((COMP_CWORD > 0)); then'
  echo $'    compwords=("${COMP_WORDS[@]:1:$((COMP_CWORD - 1))}")'
  echo $'  fi'
  echo $'  local compline="${compwords[*]}"'
  echo $''
  echo $'  COMPREPLY=()'
  echo $''
  echo $'  case "$compline" in'
  echo $'    \'bootstrap\'*\'--config\')'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(ls $HOME/.local/share/lct/config_versions/ 2>/dev/null | grep -E \'^[0-9]{4}\.[0-9]{2}\.[0-9]{2}\.tar\.xz$\' | sed \'s/\.tar\.xz$//\' | tr \'\n\' \' \')")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'lib list\'*\'--type\')'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "cargo cask flatpak formula go mas tap vscode")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'dotfiles remove\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(yq \'.dotfiles | .[]\' $LCT_CONFIG_FILE 2>/dev/null) --help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'env list\'*\'--key\')'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(yq \'keys | join(" ")\' $LCT_ENV_FILE 2> /dev/null)")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'plugin install\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'configs remove\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(yq \'.configs | .[]\' $LCT_CONFIG_FILE 2>/dev/null) --help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'plugin remove\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(yq \'.plugins | .[]\' $LCT_CONFIG_FILE 2>/dev/null) --help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'dotfiles list\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'e list\'*\'--key\')'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(yq \'keys | join(" ")\' $LCT_ENV_FILE 2> /dev/null)")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'alias remove\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(yq \'keys | join(" ")\' $LCT_ALIAS_FILE 2>/dev/null) --help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'dotfiles add\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(ls -a "$HOME" 2>/dev/null | grep -v \'^\.\.?$\' | tr \'\n\' \' \') --help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'configs list\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'bootstrap\'*\'-c\')'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(ls $HOME/.local/share/lct/config_versions/ 2>/dev/null | grep -E \'^[0-9]{4}\.[0-9]{2}\.[0-9]{2}\.tar\.xz$\' | sed \'s/\.tar\.xz$//\' | tr \'\n\' \' \')")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'lib install\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'lib list\'*\'-t\')'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "cargo cask flatpak formula go mas tap vscode")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'env list\'*\'-k\')'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(yq \'keys | join(" ")\' $LCT_ENV_FILE 2> /dev/null)")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'plugin init\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'plugin list\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'plugin load\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'configs add\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(ls "$HOME/.config" 2>/dev/null | while IFS= read -r entry; do if [[ -d "$HOME/.config/$entry" ]]; then printf \'%s \' "$entry"; fi; done) --help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'completions\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'lib remove\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'env reload\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'b\'*\'--config\')'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(ls $HOME/.local/share/lct/config_versions/ 2>/dev/null | grep -E \'^[0-9]{4}\.[0-9]{2}\.[0-9]{2}\.tar\.xz$\' | sed \'s/\.tar\.xz$//\' | tr \'\n\' \' \')")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'env unset\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'e list\'*\'-k\')'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(yq \'keys | join(" ")\' $LCT_ENV_FILE 2> /dev/null)")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'p install\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'alias add\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'bootstrap\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--config --help -c -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'lib dump\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'lib list\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help --type -h -t")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'env list\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help --key -h -k")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'e reload\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'plugin i\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'p remove\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(yq \'.plugins | .[]\' $LCT_CONFIG_FILE 2>/dev/null) --help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'dotfiles\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h add list remove")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'lib add\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'env set\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'e unset\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'configs\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h add list remove")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'e list\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help --key -h -k")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'plugin\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h i init install list load remove")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'p init\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'p list\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'p load\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'config\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(ls $HOME/.config/ 2> /dev/null | tr \'\n\' \' \') --help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'gather\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--force --help -f -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'e set\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'alias\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h add remove")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'setup\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'prune\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--cache --dry --help --plugins --unused -c -d -h -p -u")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'init\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'b\'*\'-c\')'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "$(ls $HOME/.local/share/lct/config_versions/ 2>/dev/null | grep -E \'^[0-9]{4}\.[0-9]{2}\.[0-9]{2}\.tar\.xz$\' | sed \'s/\.tar\.xz$//\' | tr \'\n\' \' \')")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'lib\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h add dump install list remove")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'env\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h list reload set unset")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'p i\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'e\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h list reload set unset")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'p\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help -h i init install list load remove")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'b\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--config --help -c -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    \'g\'*)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--force --help -f -h")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'    *)'
  echo $'      while read -r; do COMPREPLY+=("$REPLY"); done < <(compgen -W "$(_lct_completions_filter "--help --version -h -v alias b bootstrap completions config configs dotfiles e env g gather init lib p plugin prune setup")" -- "$cur")'
  echo $'      ;;'
  echo $''
  echo $'  esac'
  echo $'} &&'
  echo $'  complete -F _lct_completions lct'
  echo $''
  echo $'# ex: filetype=sh'
}